Attribute VB_Name = "modSelectAngle"
Option Explicit
...
Public Sub DrawSelecterAngle(ThePictureBox As Control, X As Single, Y As Single)
If IsDoing Then Exit Sub
IsDoing = True

'It helps if the picture box is a square
ThePictureBox.Cls
ThePictureBox.Circle (ThePictureBox.ScaleWidth / 2, ThePictureBox.ScaleHeight / 2), ((((ThePictureBox.ScaleWidth / 2) + (ThePictureBox.ScaleHeight / 2)) / 2) - 50), vbBlack 'Draw Circle
...
Midlpoint.X = ThePictureBox.ScaleWidth / 2 'Set Middlepoint
Midlpoint.Y = ThePictureBox.ScaleHeight / 2

Source.X = ThePictureBox.ScaleWidth / 2    'Set 0 Degrees
Source.Y = (ThePictureBox.ScaleHeight / 2) - ((((ThePictureBox.ScaleWidth / 2) + (ThePictureBox.ScaleHeight / 2)) / 2) - 50)

Source2.X = ThePictureBox.ScaleWidth - 100 'set 270 degrees
Source2.Y = ThePictureBox.ScaleHeight / 2
...
'thepicturebox.Line (Midlpoint.X, Midlpoint.Y)-(Mouse.X, Mouse.Y), RGB(255, 0, 0)
'thepicturebox.Line (Midlpoint.X, Midlpoint.Y)-(Source.X, Source.Y), RGB(0, 255, 0)
'thepicturebox.Line (Source.X, Source.Y)-(Mouse.X, Mouse.Y), RGB(0, 0, 255)
'thepicturebox.Line (Midlpoint.X, Midlpoint.Y)-((Source.X + Mouse.X) / 2, (Source.Y + Mouse.Y / 2))
'thepicturebox.Line (Source2.X, Source2.Y)-(Mouse.X, Mouse.Y), RGB(0, 0, 255)
'thepicturebox.Line (Source2.X, Source2.Y)-(Midlpoint.X, Midlpoint.Y), RGB(0, 255, 0)
'thepicturebox.Line (Midlpoint.X, Midlpoint.Y)-((Mouse.X + Source2.X) / 2, (Mouse.Y + Source2.Y) / 2)

TheSine = Sin(DegtoRad(270 - theAngle)) * (Source2.X - Midlpoint.X)
TheCoSine = Cos(DegtoRad(270 - theAngle)) * (Source2.X - Midlpoint.X)

ThePictureBox.Line (Midlpoint.X, Midlpoint.Y)-(Midlpoint.X + TheCoSine, Midlpoint.Y + TheSine)
...
If Mouse.Y > ThePictureBox.ScaleHeight / 2 And Mouse.X > Midlpoint.X / 2 Then
